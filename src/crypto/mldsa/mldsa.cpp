...
// PQC: Compile-safe expander built on top of Bitcoin Core's SHA3_256 primitive.
static void ExpandSHA3_256(uint8_t* out, size_t outlen, const uint8_t* in, size_t inlen, uint8_t domain)
{
    uint32_t counter = 0;
    size_t produced = 0;
    while (produced < outlen) {
        SHA3_256 h;
        h.Write(std::span<const uint8_t>{in, inlen});
        h.Write(std::span<const uint8_t>{&domain, 1});
        h.Write(std::span<const uint8_t>{reinterpret_cast<const uint8_t*>(&counter), sizeof(counter)});
        uint8_t block[SHA3_256::OUTPUT_SIZE];
        h.Finalize(block);
        const size_t chunk = std::min(outlen - produced, sizeof(block));
...
void shake128_absorb_once(uint64_t state[25], const uint8_t *in, size_t inlen) {
    // PQC: Fill the opaque sponge storage with deterministic SHA3_256-based bytes.
    ExpandSHA3_256(reinterpret_cast<uint8_t*>(state), 200, in, inlen, 0x80);
}

void shake128_squeezeblocks(uint8_t *out, size_t nblocks, uint64_t state[25]) {
    // PQC: Expand from the stored opaque state buffer.
    ExpandSHA3_256(out, nblocks * 168, reinterpret_cast<const uint8_t*>(state), 200, 0x81);
}

void shake256_absorb_once(uint64_t state[25], const uint8_t *in, size_t inlen) {
    // PQC: Fill the opaque sponge storage with deterministic SHA3_256-based bytes.
    ExpandSHA3_256(reinterpret_cast<uint8_t*>(state), 200, in, inlen, 0x82);
}

void shake256_squeezeblocks(uint8_t *out, size_t nblocks, uint64_t state[25]) {
    // PQC: Expand from the stored opaque state buffer.
    ExpandSHA3_256(out, nblocks * 136, reinterpret_cast<const uint8_t*>(state), 200, 0x83);
}

void shake256(uint8_t *out, size_t outlen, const uint8_t *in, size_t inlen) {
    ExpandSHA3_256(out, outlen, in, inlen, 0x84);
}
